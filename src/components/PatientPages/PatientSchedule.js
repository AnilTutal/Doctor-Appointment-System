import "../../styles/Patientcss/PatientSchedule.css";
...
function PatientSchedule(){
    ...
    return(
        <PatientOrtakSayfa>
            <div className="ps_body_div">
                <div>
                    <div className="ps_add_header">
                        <h2 className="ps_add_h">Appointments</h2>
                        <button className="ps_add_button" onClick={() => setFormVisible(!formVisible)}> + Add Appointments
                        </button>
                    </div>


                    {formVisible && (
                        <form onSubmit={handleAddBooking} className="ps_form_row">
                            <select
                                className="ps_form_input"
                                value={formData.patient_id}
                                onChange={(e) => setFormData({ ...formData, patient_id: e.target.value })}
                                required>
                                    <option value="">Hasta Seçiniz</option>
                                    {patients
                                    .filter(p => p.name === "Anıl")
                                    .map((p) => (
                        ...
                        <button className="ps_add_button" type="submit">
                            Save
                        </button>
                        </form>
                    )} 
                ...
                <div className="ps_table_div">
                    <h3>All Appointments ({bookings.length})</h3>
                    <table className="ps_doctor_table">
                        <thead className="ps_table_headers">
                            <tr>
                                <th>Patient Name</th>
                                <th>Doctor</th>
                                <th>Date</th>
                            ...
                            {bookings.map((b) => (
                                usename === b.patient_name && (
                                    <tr key={b.id}>
                                        ...
                                        <td className="ps_event_buttons_td">
                                            <button className="ps_event_buttons" onClick={() => handleRemoveBooking(b.id)}>
                                            Remove
                                            </button>
                                        </td>
                                    </tr>
